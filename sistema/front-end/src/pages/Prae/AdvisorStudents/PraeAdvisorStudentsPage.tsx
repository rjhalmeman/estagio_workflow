...
interface PraeAdvisorStudentsPageProps {
  ...
  onUnlinkStudent: (id: string) => void;
...
export const PraeAdvisorStudentsPage: React.FC<PraeAdvisorStudentsPageProps> = ({
  ...
  const handleUnlink = (id: string, name: string) => {
    const confirmUnlink = window.confirm(`Deseja desvincular o aluno ${name}?`);
    if (confirmUnlink) {
      onUnlinkStudent(id);
    }
  };

  const handleNavigateToTracking = (student: StudentItem) => {
    onSelectStudent({ name: student.name, company: student.company });
